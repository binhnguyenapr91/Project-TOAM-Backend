package edu.codegym.toam.service;

import edu.codegym.toam.model.*;
import edu.codegym.toam.repository.*;
...
public class DataSeedingListener implements ApplicationListener<ContextRefreshedEvent> {
    ...
    @Autowired
    private PropertiesRepository houseRepository;

    @Autowired
    private PropertyTypeRepository categoryRepository;
    ...
    public void onApplicationEvent(ContextRefreshedEvent arg0) {
        ...
        String[] userDefaultStr = new String[]{"admin", "host", "renter"};
        int j=0;
        for (String userStr : userDefaultStr) {
            if (accountRepository.findAccountByUsername(userStr) == null) {
                Account account = new Account();
                String email = userStr + "1@gmail.com";
                account.setEmail(email);
                account.setName(userStr);
                account.setUsername(userStr);
                account.setStatus(true);
                account.setPassword(passwordEncoder.encode("123456"));
                Set<Role> roles = new HashSet<>();
                ERole roleName = userStr.equals("admin") ? ERole.ROLE_ADMIN :
                        userStr.equals("host") ? ERole.ROLE_HOST:ERole.ROLE_RENTER;
...
//        String[] users = new String[]{"QuangHuy", "HoangLinh", "AlexWoo", "DangLinh"};
...
//        }
        ...
        for (String category : categories) {
            if (categoryRepository.findByName(category) == null) {
                categoryRepository.save(new PropertiesTypes(category));
            }
        }
...
//        String[] houseNames = new String[]{"Dang Linh's House", "Huynh's House", "Huy's House", "Quy's House", "Nam's House", "Tuan Vu's House", "Van Duc's House", "Binh's House", "Duong's House", "Giang Coi's House","Quang Teo's House","Hoang Linh's House"};
